...
class Chart {
  ...
  async startChart(chartData, tradeData) {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      if (i > 0) {
        label.position.x = vm.topMAContainer.width + 15
      } else {
        label.position.x = vm.topMAContainer.width
      }
      vm.topMAContainer.addChild(label)

      const num = new PIXI.Text('0000.000', {
        fill: vm.option.topMA.color[i],
        fontSize: 10
      })
    ...
    for (let i = 0; i < 3; i++) {
      ...
      if (i > 0) {
        label.position.x = vm.bottomMAContainer.width + 15
      } else {
    ...
    vm.focusLine.beginTextureFill(vm.gradient('rgba(255, 255, 255, 0.01)', 'rgba(255, 255, 255, 0.3)', vm.mainContainerHeight))
    ...
    vm.priceRightRect.drawRect(0, -2, vm.priceRightLabel.width + 15, vm.priceRightLabel.height + 4)
  ...
  async computerOnMove (event, chartData, tradeData) {
    ...
    if(chartData[tapObject]) {
      ...
      vm.toolBoxNum.children[1].text = chartData[tapObject][0]
      vm.toolBoxNum.children[2].text = chartData[tapObject][3]
      vm.toolBoxNum.children[3].text = chartData[tapObject][2]
      vm.toolBoxNum.children[4].text = chartData[tapObject][1]
      ...
      vm.toolBoxNum.children[5].text = parseFloat(chartData[tapObject][0] - chartData[tapObject][1]).toFixed(2) * -1
      ...
      if(vm.MA5Data[tapObject] !== '-') {
        vm.topMAContainer.children[1].text = parseFloat(vm.MA5Data[tapObject]).toFixed(2)
      } else {
        vm.topMAContainer.children[1].text = '-'
      }

      if(vm.MA10Data[tapObject] !== '-') {
        vm.topMAContainer.children[3].text = parseFloat(vm.MA10Data[tapObject]).toFixed(2)
      } else {
        vm.topMAContainer.children[3].text = '-'
      }

      if(vm.MA30Data[tapObject] !== '-') {
        vm.topMAContainer.children[5].text = parseFloat(vm.MA30Data[tapObject]).toFixed(2)
      } else {
        vm.topMAContainer.children[5].text = '-'
      }
  ...
  async hammerOnTap (event, chartData, tradeData) {
    ...
    if (booleanNowPriceX && booleanNowPriceY && vm.priceAllContainer.alpha && chartData.length * vm.dataSpace + vm.chartRightSpace > vm.appWidth) {
      ...
      if (chartData[tapObject]) {
        ...
        vm.toolBoxNum.children[6].text = parseFloat((chartData[tapObject][0] - chartData[tapObject][1]) / chartData[tapObject][1] * 100).toFixed(3) * -1 + '%'
        vm.toolBoxNum.children[7].text = parseFloat(tradeData[tapObject]).toFixed(3)
  ...
  getSplitData () {
    ...
    const yText_1 = vm.candleMaxData
    const yText_2 = (vm.candleMaxData * 3 / 4 + vm.candleMinData / 4)
    const yText_3 = ((vm.candleMaxData + vm.candleMinData) / 2)
    const yText_4 = (vm.candleMaxData / 4 + vm.candleMinData * 3 / 4)
    const yText_5 = vm.candleMinData
    
    yChildren[0].text = yText_1.toFixed(3)
    yChildren[1].text = yText_2.toFixed(3)
    yChildren[2].text = yText_3.toFixed(3)
    yChildren[3].text = yText_4.toFixed(3)
    yChildren[4].text = yText_5.toFixed(3)
  ...
  getPricePosY (chartData) {
    ...
    vm.priceRightContainer.children[2].text = nowprice.toFixed(3)
    vm.priceAllContainer.children[4].text = nowprice.toFixed(3)
  ...
  getMaxMinCandle () {
    ...
    // 讀入數據
    const value_max = vm.candleMaxData
    const value_min = vm.candleMinData
    ...
    vm.animation = gsap.to(lastCandleBox, {
      ...
      onUpdate: function () {
        if (this._targets[0].height > 0 ) {
          ...
          // lastCandleLine.position.x = vm.dataWidth - lastCandleLine.width / 2
        // } else if (this._targets[0].height < 0 && this._targets[0].fill.color === vm.fill.downColor) {
  ...
  getRealTimeMAValue (tradeData) {
    ...
    if(!vm.toolBox.alpha) {
      ...
      vm.bottomMAContainer.children[3].text = parseFloat(vm.bottomMA5Data[vm.bottomMA5Data.length - 1]).toFixed(3)
      vm.bottomMAContainer.children[5].text = parseFloat(vm.bottomMA10Data[vm.bottomMA10Data.length - 1]).toFixed(3)

      vm.topMAContainer.children[1].text = parseFloat(vm.MA5Data[vm.MA5Data.length-1]).toFixed(3)
      vm.topMAContainer.children[3].text = parseFloat(vm.MA10Data[vm.MA10Data.length-1]).toFixed(3)
      vm.topMAContainer.children[5].text = parseFloat(vm.MA30Data[vm.MA30Data.length-1]).toFixed(3)
    }
  }
}